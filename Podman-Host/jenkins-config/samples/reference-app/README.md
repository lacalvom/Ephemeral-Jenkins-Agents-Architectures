...
`reference-pipeline` del laboratorio `Podman-Host`.
...
`ansible/roles/podman_host/tasks/main.yml`), y el pipeline los compila y
empaqueta mediante agentes efímeros de Podman.
...
2. **Construcción Frontend**: `npm install && npm run build` dentro de un
   contenedor `ubi9/nodejs-20` → genera
