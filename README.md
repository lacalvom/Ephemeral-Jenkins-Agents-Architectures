...
│   └── roles/
│       ├── common/                        # Base: usuario jenkins, paths, paquetes
│       ├── jenkins_controller/             # Instala Jenkins LTS + plugins + scripts
│       ├── podman_host/                   # Instala Podman + java + servicio del agente + app de ejemplo
│       ├── agent_registration/            # Lee el secret del Controller y arranca el agente
│       └── podman_secrets_tooling/        # Tooling de cifrado para Podman Secrets (Fase 5, siempre se aplica)
├── jenkins-config/                        # Configuracion inmutable de Jenkins
│   ├── plugins.yaml                       # Lista declarativa de plugins con versiones
│   ├── casc/
│   │   └── jenkins.yaml                   # Referencia de JCasC (NO se usa actualmente)
│   ├── groovy/                            # Scripts Groovy para init.groovy.d/
│   │   ├── create-admin.groovy             #   01 - Crea el usuario admin
│   │   ├── create-agent.groovy             #   02 - Crea el nodo permanente
│   │   ├── create-pipeline.groovy          #   03 - Crea el job reference-pipeline
│   │   └── create-config-files.groovy      #   04 - Crea los Managed Config Files (Maven/npm)
│   ├── jobs/
│   │   └── reference-pipeline.groovy       # Definicion del pipeline de ejemplo
│   └── samples/
│       └── reference-app/                 # App de ejemplo que compila el pipeline
